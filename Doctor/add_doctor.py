...
def add_doc(
    ...
    if not full_name or not role or not address:
    ...
    try:
        ...
        query = """
        INSERT INTO hms_add_doctor(
            full_name, roles, user_name, email, password,
            date, gender, address, country, city, State,
            postal, phone_number, image, bio, stataus
        )
        VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        """

        data= (
            full_name,role,user_name,email,hashed_password,
            date_of_birth,gender,address,country,city,state,
            postal,phone,avatar_path,bio,status
        )
        cur.execute(query,data)
        con.commit()

        messagebox.showinfo("Success","Doctor register successfully")
        cur.close()
        con.close()
...
def add_doctor_frame(content_frame):
    ...
    create_Label('Full Name',row=1,col=0,required=True)
    create_Label('Role', row=1, col=2, required=True)

    full_name_ent = create_Entry(2,0,2)
    role_ent = create_Entry(2,2,2)
...
#==================Choose file Button====================================
    ...
    bio_txt.grid(row=16,column=0,columnspan=4,sticky="we")
